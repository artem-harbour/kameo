...
export class FormSettingsRadioOptions extends LitElement {
  ...
  _handleAttributeUpdate({ event, attr, value, index }) {
    ...
    const radioOptions = { node: this.node, pos };
    const attrs = { [attr]: value };

    this.editor.commands.updateFormRadioOption(radioOptions, index, attrs);
  }
  ...
  _handleAddClick() {
    ...
    this.editor.commands.addFormRadioOption(radioOptions, option);
  }
  ...
  _handleDeleteClick(index) {
    ...
    this.editor.commands.removeFormRadioOption(radioOptions, index);
  }
